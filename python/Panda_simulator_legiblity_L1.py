...
class ObstacleAvoidance2(crocoddyl.CostModelAbstract):
    def __init__(self, state, frameId, Q2, eigen_vec, traj_weights = None, traj=None, nu=None):
...
X = X-X.mean() #np.reshape(np.mean(X,axis=1),[3,1])
...
traj_weights[:,0] = np.linspace(1,0,len(traj))**2
...
obsAvoidCost = ObstacleAvoidance2(state, eeIdx, Q2, eigen_vec=eigVec, traj=traj, traj_weights=traj_weights)
...
runningCostModel.addCost("obstacleAvoidance", obsAvoidCost, 0.5)
