...
class ObstacleAvoidance2(crocoddyl.CostModelAbstract):
    def __init__(self, state, frameId, Q2, traj_weights = None, traj=None, nu=None):
    ...
    def calc(self, data, x, u): 
        ...
        for repPoint, w,  in zip(self.traj, self.traj_weights):
            ...
            d = (xd-repPoint).transpose() @ self.Q2 @ (xd-repPoint)
            if d > 1e-6:
                c += -w*math.log(d)
        data.cost = c
    ...
    def calcDiff(self, data, x, u): 
        ...
        for repPoint, w in zip(self.traj, self.traj_weights):
            ...
            if (d > 1e-6).any():
                Jx[:,0] += (-2*w*self.Q2 @ d) / (d.transpose() @ self.Q2 @ d)
        ...
        for repPoint, w in zip(self.traj, self.traj_weights):
            d = xd-repPoint
            if (d > 1e-6).any():
                Jxx += -w*((2*self.Q2)/(d.transpose() @ self.Q2 @ d) - (4*(self.Q2 @ d)@(self.Q2 @ d).transpose())/((d.transpose() @ self.Q2 @ d)**2))
        
        Lxx = np.zeros((nq+nv, nq+nv))
        Lxx[0:nq,0:nq] = J[:3,:].transpose() @ Jxx @ J[:3,:]
        data.Lxx = Lxx
...
traj_weights = np.ones(len(traj))*5
#traj_weights = np.linspace(10,0,len(traj))
...
obsAvoidCost = ObstacleAvoidance2(state, eeIdx, Q2, traj=traj, traj_weights=traj_weights)
...
terminalCostModel.addCost("gripperPose", goalTrackingCost, 800000)
...
terminalCostModel2.addCost("gripperPose", goalTrackingCost, 20000)
