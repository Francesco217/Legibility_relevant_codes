function Manipulator_all_joints_nom_end_pos
%% Parameters
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
param.w2(1:30) = linspace(1,0.1,30);
...
param.Q2 = zeros(param.nbVarX*param.nbData, param.nbVarX*param.nbData, length(param.idx));

qf = param.nbData*10000;
Mat1 = eye(param.nbData)*1;
Mat1(param.nbData, param.nbData) = qf;
param.Q2 = kron(Mat1, eye(param.nbVarX));
...
%}
    ...
    for t = 1:param.nbData         % iterating over the points of the trajectory
        ...
        Q2 = param.Q2((t*Nq-Nq+1):t*Nq,(t*Nq-Nq+1):t*Nq);
        cost = cost + (q(:,t)-param.qN)'*Q2*(q(:,t)-param.qN);
    end
...
%}
    ...
    for i = 1:param.nbData
        ...
        Q2 = param.Q2((i*Nq-Nq+1):i*Nq,(i*Nq-Nq+1):i*Nq);
        Hq(i*Nq-(Nq-1):i*Nq,i*Nq-(Nq-1):i*Nq) = Hq(i*Nq-(Nq-1):i*Nq,i*Nq-(Nq-1):i*Nq) + Q2;
    end
