function q = end_position_L1(param)
%{
...
for k = 1:length(param.idx)
   if k == length(param.idx)
        param2.Q1(:,:,k) = eye(param.nbVarF)*100;
   else
       param2.Q1(:,:,k) = eye(param.nbVarF)*0;
   end
end
...
epsilon = 1E-4 * diag(ones(1,param.nbVarF));
...
function  cost = cost_fct(q, param, param2)
   ...
   for k = 1:length(param.idx)
      ...
      if not(k == length(param.idx))
        for n = 2:param.nbObj
            X2 = F(:,k) - param.Obj3d(:,n);
            cost = cost - param2.w2*abs(sum(X2));
        end
      end
...
for k = 1:length(param.idx)
   ...
   if not(k == length(param.idx))
       for n = 2:param.nbObj
          ...
          I_neg = X2 <= 0;
          I_pos = X2 > 0;
          J1(I_neg) = J1(I_neg) + param.w2;
          J1(I_pos) = Jx(I_pos) - param.w2;
       end
   end
