function Manipulator_all_joints
...
param.w2(1:30) = linspace(10,0.1,30);
qf = param.nbData*param.w2(1)*50;       % terminal cost for reaching
Mat1 = eye(param.nbData);
Mat1(param.nbData, param.nbData) = qf;
param.Q1 = kron(Mat1, eye(param.nbVarF)*1000);    % reaching part of the cost
...
%param.Obj3d = [[0.8;0.1;0.5], [0.8; -0.1;0.5]];  
...
idx = [2, 4, 5, 6, 8, 9];       % idx we care about (ignore basis and 3,7 bcs redundant)

e = 1E-6;
Q = repmat(q, [1, param.nbVarX]);
Qe = Q + eye(param.nbVarX) * e;

J = zeros(param.nbVarF, param.nbVarX, length(idx));
...
%}
    ...
    Q1 = param.Q1(1:param.nbVarF, 1:param.nbVarF);
...
%}
    ...
    for i = 1:param.nbData
        ...
        for k = 1:length(idx)
            ...
            Hx_i = 2*param.Q1(1:param.nbVarF, 1:param.nbVarF);
