...
function Log_cost_projected_approach2
    ...
    for k = 1:param.nbData          % iterate over the points of trajectorx
        ...
        for j = 1:param.nbObj-1     % iterate over the number of different objectives
            for i = 2:param.nbData  % iterate over the trajectory to avoid
                ...
                X2 = Xp(:,k)-param.traj(:,i,j);
                Jxp(2*k-1:2*k,:) = Jxp(2*k-1:2*k,:) - param.w2(i)*2*Q2*X2/((X2'*Q2*X2+1));
    ...
    for k = 1:M
       ...
       for i = 2:param.nbData
           ...
           for j = 1:param.nbObj-1
               X2 = Xp(:,k) - param.traj(:,i,j);
               Jxp(:,k) = Jxp(:,k) - param.w2(i)*2*Q2*X2/(X2'*Q2*X2+1);
               Jx(:,k) = Jx (:,k)+ P'*Jxp(:,k);
           end
       end
    ...
    for k = 1:param.nbData
        ...
        for j = 1:param.nbObj-1         % iterate over the number of different objectives
            for i = 2:param.nbData      % iterate over the trajectory    
                ...
                X2 = Xp(:,k)-param.traj(:,i,j);
                H2d = H2d - param.w2(i)*((2*Q2/((X2'*Q2*X2+1)) - 4*Q2*X2*(Q2*X2)'/((X2'*Q2*X2+1)^2))); 
...
function cost = cost_fct(X, U, param, R)
    ...
    for k = 1:param.nbData
        ...
        for j = 1:param.nbObj-1        %iterating over points of the trajectory
           for i = 2:param.nbData      %iterating over the number of objectives

                X2 = Xp(:,k)-param.traj(:,i,j);
                cost = cost - param.w2(i)*log((X2'*Q2*X2+1));
    ...
    for k = 1:M    
       ...
       for i = 1:param.nbData
           for j = 1:param.nbObj-1
               ...
               cost(:,k) = cost(:,k)-param.w2(i)*log(X2'*Q2*X2+1);
           end
       end
...
% funcitons used fro report demonstration 

function Jx = Gradient_x_test(X, param)
    %{ 
    Computes the gradient of the cost function with respect to the x
    X: (NxM) datapoints
    Jx: (NMx1) vector containing the gradient of each step
    ...
    for k = 1:param.nbData          %iterate over the points of trajectorx
        ...
        for j = 1:param.nbObj-1     %iterate over the number of different objectives
              %iterate over the trajectory to avoid

                X2 = Xp(:,k)-param.Obj(:,2);
                Jxp(2*k-1:2*k,:) = Jxp(2*k-1:2*k,:) - param.w2(1)*2*Q2*X2/((X2'*Q2*X2));
    %             Jxp = param.w2(i)*2*param.Q2*X2/((X2'*Q2*X2));
    %             Jxp3 = reshape(Jxp, param.nbVarX-1, param.nbData);
    %             Jxp3 = reshape(P'*Jxp3, param.nbVarX*param.nbData,1);
    %             Jx = Jx + Jxp3;
...
function cost = cost_fct_test(X, U, param, R)
    %{
    X (NxM)
    ...
    for k = 1:param.nbData
        Q2 = param.Q2(2*k-1:2*k,2*k-1:2*k);
        for j = 1:param.nbObj-1        %iterating over points of the trajectory
            %iterating over the number of objectives

            X2 = Xp(:,k)-param.Obj(:,2);
            cost = cost - param.w2(1)*log((X2'*Q2*X2));
...
function Hxx = Hess_x_test(X, param)
    %{
    Returns the Hessian of the cost function around each one of the
    datapoints
    X (NxM) input data
    ...
    for k = 1:param.nbData
        ...
        for j = 1:param.nbObj-1         %iterate over the number of different objectives
            ...
            %Q2 = param.Q2(2*k-1:2*k,2*k-1:2*k);
            X2 = Xp(:,k)-param.Obj(:,2);
            H2d = H2d - param.w2(1)*((2*Q2/((X2'*Q2*X2)) - 4*Q2*X2*(Q2*X2)'/((X2'*Q2*X2)^2))); 
